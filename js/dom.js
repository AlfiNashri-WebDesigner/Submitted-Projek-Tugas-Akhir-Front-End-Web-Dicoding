...
function addBook() {   //tambah buku
	...
	const waktu = document.getElementById("inputTarget").value;  //waktu selesai
    ...
    console.log("Waktu Selesai: "+ waktu); //waktu selesai
   
    console.log("Tahun: "+ inputTahunBuku);
   
    console.log("Selesai Dibaca: "+ inputDibaca);

	//input tambahkan ke local storage
    const book = inputBook(inputJudulBuku, inputPenulisBuku, waktu, inputTahunBuku, inputDibaca);
    
	const bookObject = composeBookObject(inputJudulBuku, inputPenulisBuku, waktu, inputTahunBuku,  inputDibaca);
    ...
    updateDataToStorage();   //simpan
}


function inputBook(inputJudul, inputPenulis, waktu, inputTahun, inputDibaca){
   ...
   // detailnya
    ...
    penulisBuku.innerText = "Penulis:  " +  inputPenulis;   //tambahkan keterangan
	
	
	// Merangkai elemen Waktu Selesai buku
    const waktubuku = document.createElement('p');
    waktubuku.classList.add('book-details');
    waktubuku.innerText = "Waktu Selesai:  " + waktu;

    // Merangkai elemen tahun terbit buku
    const tahunBuku = document.createElement('p');
    tahunBuku.classList.add('book-details');
    tahunBuku.innerText = "Tahun:  " + inputTahun;
    ...
    bookContainer.append(judulBuku, penulisBuku, waktubuku, tahunBuku, buttons);  //tambahkan di halaman
...
function addBookToCompleted(taskElement) {
    ...
    const newBook = inputBook(book.judul, book.penulis, book.waktu, book.tahun, inputDibaca=true);
...
function editBook(taskElement) {
	...
	//tambah
	const editWaktuBuku = document.getElementById("editWaktuBuku");
    editWaktuBuku.value = book.waktu;   //book waktu
    ...
    submitEdit.addEventListener('click', function(event) {
        ...
        updateEditBook(editJudulBuku.value, editPenulisBuku.value, editWaktuBuku.value, editTahunBuku.value, editDibaca.checked, book.id);
...
function updateEditBook(judul, penulis, waktu, tahun,  dibaca, id) {
...
function undoBookFromCompleted(taskElement){
    ...
    // Membuat data buku baru                             //tambahkan
    const newBook = inputBook(book.judul, book.penulis, book.waktu, book.tahun,  book.isCompleted);
    newBook[BOOK_ITEM_ID] = book.id;
